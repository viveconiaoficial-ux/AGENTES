...
export default async function VistaPortalConversacionesPage({
  params,
}: {
  ...
  if (error) {
    return (
      <div className="glass rounded-2xl px-5 py-4 text-sm text-red-300">{error.message}</div>
    );
  }
    ...
    <div className="space-y-6">
      <header>
        <div className="text-[11px] font-medium uppercase tracking-[0.22em] text-white/40">
          Tu negocio · Conversaciones
        </div>
        <h1 className="mt-2 text-3xl font-semibold tracking-tight">
          {negocio.nombre || "Sin nombre"}
        </h1>
        <p className="mt-2 text-sm text-white/55">
          Historial de mensajes entre tus clientes y el asistente (web o WhatsApp).
        </p>
      </header>
      ...
      <p className="text-[12px] text-white/35">
        ¿Necesitas el panel completo de la agencia?{" "}
        <Link href={`/clientes/${params.clienteId}/citas`} className="text-white/55 underline">
          Ir a citas (panel agencia)
        </Link>
      </p>
    </div>
  );
